...
void insert_variable(VarInfo var)
{
	...
	/*
	SymbolInfo *existence = symboltable->lookup_current(var.var_name);
	
	if(existence != NULL){
		existence->setType("ID");
		existence->setVarType(var.var_type);
		existence->setSelfType(var.self_type);
		existence->setVarSize(var.var_size);
	}
	else{
